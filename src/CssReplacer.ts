...
export class CssReplacer {
    ...
    async replaceStyleSheets() {
        console.log(`[BeautySelectorAddon][CssReplacer] replaceStyleSheets() start.`);
        let countOk = 0;
        let countError = 0;
        let countFind = 0;
        let countReplaced = 0;
        ...
        for (let i = 0; i < styleSheets.length; i++) {
            ...
            try {
                            ...
                            if (st[key]) {
                                ...
                                if (v && v.includes("url(")) {
                                    ...
                                    if (this.checkIsNeedReplace(v)) {
                                        ...
                                        if (r !== v) {
                                            console.log(`[BeautySelectorAddon][CssReplacer] replaceStyleSheets() replace`, [v, r]);
                                            st[key] = r;
                                            ++countReplaced;
                                        }
                                    }
                ...
                if (e instanceof DOMException &&
                    ...
                    console.log(`[BeautySelectorAddon][CssReplacer] This is a cross-origin stylesheet. Cannot access. ignore.`, [styleSheet.href]);
                } else {
                    console.warn(`[BeautySelectorAddon][CssReplacer] Could not access rules for stylesheet: ${styleSheet.href}`);
                    console.warn(e);
                }
            }
        }
        console.log(`[BeautySelectorAddon][CssReplacer] replaceStyleSheets() done. `
        ...
        this.logger.log(`[BeautySelectorAddon][CssReplacer] replaceStyleSheets() done. `
            + `countOk[${countOk}] countError[${countError}] countFind[${countFind}] countReplaced[${countReplaced}]`);
    }
    ...
    async getCorrectUrl(cssUrl: string) {
        ...
        if (!imageUrl) {
            // never go there
            console.error(`[BeautySelectorAddon][CssReplacer] getCorrectUrl() cannot match imageUrl, never go there.`, [cssUrl]);
            this.logger.error(`[BeautySelectorAddon][CssReplacer] getCorrectUrl() cannot match imageUrl, never go there. [${cssUrl}]`);
            return cssUrl;
        }
